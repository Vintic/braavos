component extends="Controller" output=false {
	...
	public any function update() {
		...
		local.where = ["id = #params.key#", "contactId = #params.contactId#"];
		local.numberOfParameters = 2;
		local.criteria = model("Criteria").findOne(where = whereify(local.where), parameterize = local.numberOfParameters);
		if (IsObject(local.criteria)) {
			...
			if (local.criteria.update()) {
				...
				if (!local.criteria.valid()) {
					...
					local.rsp.messages.append(["There was a problem updating the criteria.", "error"]);
				}
			}
		} else {
